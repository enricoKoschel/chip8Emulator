...
//string game = "test_jump.ch8";
string game = "c8games/tetris";

HANDLE initializeScreen(int screenWidth, int screenHeight) {
	const HANDLE console = CreateConsoleScreenBuffer(GENERIC_READ | GENERIC_WRITE, 0, NULL, CONSOLE_TEXTMODE_BUFFER, NULL);
	SetConsoleActiveScreenBuffer(console);

	// disable blinking cursor in top left corner
...
private:
	...
	WORD getKeysPressed() {
		WORD keys = 0;
		keys |= (GetAsyncKeyState(0x30) & 0x8000) >> 15; // 0 KEY
		keys |= (GetAsyncKeyState(0x31) & 0x8000) >> 14; // 1 KEY
		keys |= (GetAsyncKeyState(0x32) & 0x8000) >> 13; // 2 KEY
		keys |= (GetAsyncKeyState(0x33) & 0x8000) >> 12; // 3 KEY
		keys |= (GetAsyncKeyState(0x34) & 0x8000) >> 11; // 4 KEY
		keys |= (GetAsyncKeyState(0x35) & 0x8000) >> 10; // 5 KEY
		keys |= (GetAsyncKeyState(0x36) & 0x8000) >>  9; // 6 KEY
		keys |= (GetAsyncKeyState(0x37) & 0x8000) >>  8; // 7 KEY
		keys |= (GetAsyncKeyState(0x38) & 0x8000) >>  7; // 8 KEY
		keys |= (GetAsyncKeyState(0x39) & 0x8000) >>  6; // 9 KEY
		keys |= (GetAsyncKeyState(0x41) & 0x8000) >>  5; // A KEY
		keys |= (GetAsyncKeyState(0x42) & 0x8000) >>  4; // B KEY
		keys |= (GetAsyncKeyState(0x43) & 0x8000) >>  3; // C KEY
		keys |= (GetAsyncKeyState(0x44) & 0x8000) >>  2; // D KEY
		keys |= (GetAsyncKeyState(0x45) & 0x8000) >>  1; // E KEY
		keys |= (GetAsyncKeyState(0x46) & 0x8000) >>  0; // F KEY
	...
	void opDXYN(WORD opcode) {
		...
		for (int i = 0; i < (opcode & 0x000F); i++) {
			for (int j = 0; j < 8; j++) {
				...
				if ((screen[pixel] == WHITE && ((memory[I + i] << j) & 0b10000000))) {
					screen[pixel] = BLACK;
					registers[0xF] = 1;
				}
				else if (screen[pixel] == BLACK && ((memory[I + i] << j) & 0b10000000)) {
					screen[pixel] = WHITE;
					registers[0xF] = 0;
				}
			}
...
public:
	...
	void clockCPU() {
		...
		if (soundTimer != 0) Beep(500, soundTimer * 16.66); soundTimer = 0;
	}
};

int main() {
	// console output variables
	const HANDLE console = initializeScreen(screenWidth, screenHeight);
